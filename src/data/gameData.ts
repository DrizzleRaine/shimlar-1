...
export class Player extends BattleCapable {
    ...
    public swordDef: integer = 1;
    public swordSpd: integer = 1;
    ...
    async doAttack(stage: BattleStage, preSelectedTarget?: BattleCapable) {
        ...
        const amount = Random.roll(10) + this.stats.attack + this.swordAtk;
    ...
    async decideDamage(amount: number) {
        amount = amount - this.stats.defence - this.swordDef;
        if (amount < 1) {
            amount = 1;
        }
